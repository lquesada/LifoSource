...
function iweb() {
  ...
  if (($us['clan'] != '(ninguno)') && ($numrows)) {
    ...
    $numriws = mysql_num_rows($retvil);
    $nummiembros = mysql_num_rows($retvol);
    ...
    for ($tx = 0;$tx < $nummiembros;$tx++) {
      ...
      if ($time-$r[6]<600)
        ...
        $est = '-';
      echo "<tr><td>{$est}</td><td>{$r[2]}</td><td><img src=\"{$imgroot}img/{$r[3]}.gif\" alt=\"insignia\"/></td><td>{$r[0]}</td><td>{$r[1]}</td><td>{$r[5]}/{$r[4]}</td>";
      if ($jug == $r[0])
    ...
    if ($numriws) {
      ...
      for ($tx = 0;$tx < $numriws;$tx++) {
        $r = mysql_fetch_row($retvil);
        echo "<tr><td>{$r[2]}</td><td><img src=\"{$imgroot}img/{$r[3]}.gif\" alt=\"insignia\"/></td><td>{$r[0]}</td><td>{$r[1]}</td><td>{$r[5]}/{$r[4]}</td><td>";
  ...
  if (($us['clan'] != '(ninguno)') && (!$numrows)) {
    ...
    echo "Estos son los miembros de tu clan.<br/><br/>";
    echo '<table class="highscores"><tr><th>Estado</th><th>Puntos</th><th>Insig.</th><th>Nombre</th><th>Nivel</th><th>Combates ganados/totales</th><th></th><th></th></tr>';
    $lider = mysql_fetch_row($retlider);
    mysql_free_result($retlider);
    ...
    for ($tx = 0;$tx < $nummiembros;$tx++) {
      ...
      if ($time-$r[6]<600)
        ...
        $est = '-';
      echo "<tr><td>{$est}</td><td>{$r[2]}</td><td><img src=\"{$imgroot}img/{$r[3]}.gif\" alt=\"insignia\"/></td><td>{$r[0]}</td><td>{$r[1]}</td><td>{$r[5]}/{$r[4]}</td><td>";
      if ($lider[0] == $r[0])
        echo '<b>Lider</b>';
      echo "</td></tr>";
    }
  ...
  if (isset($_REQUEST['aceptar'])) {
    if ((isset($_REQUEST['tag'])) && (isset($_REQUEST['usuario']))) {
      ...
      if ($tag == $gtag) {
        ...
        $numrows2 = mysql_num_rows($retval3);
        $numrows3 = mysql_num_rows($retval4);
        $numrows4 = mysql_num_rows($retval5);
    ...
    if (isset($_REQUEST['tag'])) {
      ...
      if ($tag == $gtag) {
        ...
        if (($ret[0] != '(ninguno)') && ($numrows2)) {
          ...
          $numrows = mysql_num_rows($retvalo);
